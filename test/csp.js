...
describe('put', () => {
  ...
  describe('that is parked', () => {
    ...
    mocha.it('should be moved to the buffer when a value is taken from it', done => {
      const ch = chan(1);
      let count = 0;
...
describe('offer and poll', () => {
  ...
  mocha.it('should succeed if they can be completed immediately by a buffer', () => {
    const ch = chan(2);
  ...
  mocha.it('should succeed if they can be completed immediately by a pending operation', () => {
    const putCh = chan();
    putAsync(putCh, 42);
  ...
  mocha.it('should fail if there are pending same-direction operations on a channel', () => {
    const putCh = chan();
    putAsync(putCh, 42);
...
describe('alts', () => {
  ...
  describe('synchronization (at most once guarantee)', () => {
    once('taking from a queued put', function* (ch1) {
      putAsync(ch1, 2);
    }, [chan(), takeReadyFromPut(1)]);

    once('taking from the buffer', function* (ch1) {
      putAsync(ch1, 2);
    }, [chan(), takeReadyFromBuf(1)]);

    once('taking from a closed channel', function* (ch1) {
      putAsync(ch1, 2);
    }, [chan(), closed(chan)]);

    once('putting to a queued take', function* (ch1) {
      takeAsync(ch1, noOp);
    }, [[chan(), 1], [putReadyByTake(), 2]]);

    once('putting to the buffer', function* (ch1) {
      takeAsync(ch1, noOp);
    }, [[chan(), 1], [putReadyByBuf(), 2]]);

    once('putting to a closed channel', function* (ch1) {
      takeAsync(ch1, noOp);
...
describe('Goroutine', () => {
  ...
  it('should leave yielded normal values untouched', function* () {
    const lst = [42, [42], { x: 42 }, '', null, undefined, true, false,
      () => {
      }, function* () {}];
